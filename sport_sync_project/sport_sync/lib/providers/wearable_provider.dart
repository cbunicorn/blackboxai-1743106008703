...
class WearableProvider with ChangeNotifier {
  ...
  Future<List<PerformanceData>> fetchBasicHistoricalData({
    ...
    try {
      final data = await _wearableService.fetchBasicHistoricalData(
        userId: 'current_user_id', // Replace with actual user ID
        deviceId: _connectedDeviceId!,
        startDate: startDate,
        endDate: endDate,
  ...
  // Fetch advanced historical data
  Future<List<PerformanceData>> fetchAdvancedHistoricalData({
    ...
    try {
      final data = await _wearableService.fetchAdvancedHistoricalData(
        userId: 'current_user_id', // Replace with actual user ID
        deviceId: _connectedDeviceId!,
        startDate: startDate,
        endDate: endDate,
  ...
  // Fetch historical data (maintain backward compatibility)
  Future<List<PerformanceData>> fetchHistoricalData({
    ...
    // Default to basic metrics for backward compatibility
    return fetchBasicHistoricalData(
      startDate: startDate,
      endDate: endDate,
    );
  }
